#!/usr/bin/env node
/**
 * agent-target.js   ("target side" — runs next to the real Bedrock server)
 *
 * This machine can be behind NAT/firewall — it only makes OUTBOUND
 * connections to the relay, so no inbound port forwarding is needed here.
 *
 * It binds its own local UDP socket to LOCAL_PORT (default 21116) — just
 * the source port used to talk to relay-server.js, independent of the
 * relay's public port — registers with the relay using a shared AUTH_KEY,
 * then for every wrapped client packet it receives, forwards the raw
 * payload to the real Bedrock server (TARGET_HOST:TARGET_PORT) via a
 * per-session local socket (OS-assigned, no conflicts there either), and
 * relays the server's replies back.
 *
 * Usage:
 *   RELAY_HOST=1.2.3.4 RELAY_PORT=21115 LOCAL_PORT=21116 \
 *   TARGET_HOST=127.0.0.1 TARGET_PORT=19132 AUTH_KEY=changeme \
 *   node agent-target.js
 *
 * Env vars:
 *   RELAY_HOST   public IP/hostname of relay-server.js
 *   RELAY_PORT   relay's UDP port (default 21115) — the port players connect to
 *   LOCAL_PORT   local UDP port this agent binds to, to talk to the relay
 *                (default 21116). This is INDEPENDENT of RELAY_PORT — it does
 *                NOT need to match. It only needs to be a free port on this
 *                machine, and only needs to differ from RELAY_PORT if the
 *                relay and agent happen to run on the same host.
 *   TARGET_HOST  where the real bedrock_server process is listening (default 127.0.0.1)
 *   TARGET_PORT  bedrock_server's UDP port (default 19132)
 *   AUTH_KEY     shared secret, must match relay-server.js
 */
...
const LOCAL_PORT = parseInt(process.env.LOCAL_PORT || '21116', 10); // independent of RELAY_PORT
...
// Must match relay-server.js's COMPRESSION_MIN_SIZE setting — off (0) by
// default. See the comment in relay-server.js for why compression usually
// doesn't help this kind of traffic.
...
// ---- Wire protocol (must match relay-server.js) ----
...
function getOrCreateSession(id) {
  ...
  session = { socket: localSocket, lastActive: Date.now() };
...
function closeSession(id) {
  ...
  if (!session) return;
  try { session.socket.close(); } catch (_) { /* noop */ }
  sessions.delete(id);
  console.log(`[-] session ${id} closed (${sessions.size} active)`);
}

mainSocket.on('message', (msg, rinfo) => {
  if (rinfo.address !== RELAY_HOST || rinfo.port !== RELAY_PORT) return; // ignore strangers
  ...
  if (msg[0] === TYPE_DATA_TO_AGENT || msg[0] === TYPE_DATA_TO_AGENT_Z) {
    const { sessionId: id, payload } = decodeDataFrame(msg);
...
// Keep retrying registration until acked, then send periodic keepalives
setInterval(() => {
  if (!registered) {
    sendHello();
  } else {
